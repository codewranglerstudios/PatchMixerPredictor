...
class Exp_Main(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        print(f'exp/main self.args.cols {self.args.cols} ')
    ...
    def train(self, setting, load=False):
        ...
        folder_path = './training_results/' + setting + '/'
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # encoder - decoder
                if self.args.use_amp:
                    print(f"using automatic mixed precision training")
                    with torch.cuda.amp.autocast():
                        if 'Linear' in self.args.model or 'TST' in self.args.model or 'Mixer' in self.args.model or 'P_' in self.args.model:
                            outputs = self.model(batch_x)
                        else:
                    ...
                    print(f"not using automatic mixed precision training")
                    ...
                    if 'Linear' in self.args.model or 'TST' in self.args.model or 'Mixer' in self.args.model or 'P_' in self.args.model:
                        ...
                        print(f"output_attention set to {self.args.output_attention}")
                        if self.args.output_attention:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                        else:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_y)
                    print(f"data shapes. outputs: {outputs.shape} batch_y: {batch_y.shape}")
                    f_dim = -1 if self.args.features == 'MS' else 0
                    outputs = outputs[:, -self.args.pred_len:, f_dim:]
                    batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                    loss = criterion(outputs, batch_y)
                ...
                time_now = time.time()
                if i % 5 == 0:
                    # Create a copy of the pred tensor for plotting
                    pred = outputs.detach().clone()
                    true = batch_y.detach().clone()
                    # Re-attach the original pred tensor to the computational graph
                    ...
                    plot_path = f"{folder_path}training_convergence_epoch_{epoch}_of_{self.args.train_epochs}_attempt_{i}.pdf"
                    visual(gt, pd, plot_path)
                    print(plot_path)
    ...
    def test(self, setting, test=0, load=False):
        test_data, test_loader = self._get_data(flag='test')
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 5 == 0:
                    ...
                    plot_path = f"{folder_path}testing_convergence_epoch_{i}.pdf"
                    visual(gt, pd, plot_path)
    ...
    def predict(self, setting, load=False):
        ...
        preds = np.array(preds)
        preds = preds.reshape(-1, preds.shape[-2], preds.shape[-1])
        preds_close = preds.squeeze()

#        if self.args.inverse:
        preds_close = pred_data.inverse_transform(preds_close)
        preds_close = preds_close[:,-1]
        ...
        np.save(folder_path + 'real_prediction.npy', preds_close)
        visual(preds_close.flatten(),preds_close.flatten(),name=f'{folder_path}final_real_prediction.pdf')
        return preds_close
